...
class HmacBased {
  generatePasscode(options, config) {
    ...
    const hmac = crypto__default.createHmac(config.algo, secretBytes);
    hmac.update(buf);
...
const generateSecret = (secretSize = DEFAULT_TOTP_SECRET_SIZE) => {
  const bytes = Buffer.from(crypto__default.randomBytes(secretSize));
...
const generateBackupCodes = (numCodes = 10, codeLength = DEFAULT_TOTP_DIGITS) => {
  ...
  for (let i = 0; i < numCodes; i++) {
    ...
    for (let j = 0; j < codeLength; j++) {
      code += crypto__default.randomInt(0, 10).toString();
    }
